...
export const updatePassword = async (  req: Request,
  ...
  try {
    ...
    hash(newPassword, 9, async(err, hash) => {
      if (err) {
        return next({
...
export const renderResetPassword = async (  req: Request,
  res: Response,
  next: NextFunction) => {
  try {
    const { userId, token = '123' } = req.query;
    let user = await userModel.findById(userId).lean();
    if (!user) {
      res.status(404).json({
        message:
          "Please ensure you have received a mail and try again in 3 minutes",
      });
      return 
    }
    await res.render("newpassword", {
            token,
            name: `${user.fname ?? ''}`
    } )
  } catch (error) {
   res.render("reset-password-failed")
  }
};
